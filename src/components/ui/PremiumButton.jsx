export default function PremiumButton({ children, icon: Icon, variant = 'gold', className = '', ...props }) {
  ...
  return (
    <button
      className={`focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-[8px] px-4 text-sm font-bold transition duration-300 ${variants[variant]} ${className}`}
      {...props}
    >
      {Icon && <Icon size={17} />}
      {children}
